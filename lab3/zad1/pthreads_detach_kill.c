...
int main()
{
    ...
    pthread_join(tid,&wynik);//ogarniecie wyniku
	...
	/*Tu wstaw kod tworzenia watku z domysllnymi wlsasnosciami*/
    pthread_create(&tid,NULL,zadanie_watku,NULL);

	sleep(2); // czas na uruchomienie watku //WHY?

	printf("\twatek glowny: odlaczenie watku potomnego\n");
	//Instrukcja odlaczania
    pthread_detach(tid);
