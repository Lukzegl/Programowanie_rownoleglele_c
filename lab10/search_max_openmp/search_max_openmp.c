...
#pragma omp parallel default(none) shared(a_max, A, p, k)
  {
#pragma omp single
    {
      int num_threads = omp_get_num_threads();
      int n = k - p + 1;
      int num_tasks = 2 * num_threads;
      int n_loc = ceil((double)n / num_tasks);
...
#pragma omp task default(none) firstprivate(p_task, k_task, A) \
    shared(a_max)
        {
...
#pragma omp task default(none) firstprivate(A, p, s, level) \
    shared(a_max_1) if(level < max_level)
    {
      a_max_1 = bin_search_max_task(A, p, s, level + 1);
    }

#pragma omp task default(none) firstprivate(A, s, r, level) \
    shared(a_max_2) if(level < max_level)
    {
...
#pragma omp parallel default(none) shared(A, p, k, a_max)
  {
#pragma omp single
    {
#pragma omp task
