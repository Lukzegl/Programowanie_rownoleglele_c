...
int main(int argc, char** argv) {
    ...
    if (size > 1) {
        ...
        n_wier = ceil(WYMIAR / (double)size);
        ...
        // Rozproszenie macierzy a za pomocą MPI_Scatter (komunikacja grupowa)
        MPI_Scatter(a, n_wier * WYMIAR, MPI_DOUBLE, a_local, n_wier * WYMIAR, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        
        // Rozesłanie wektora x do wszystkich procesów
        ...
        // Wymiana fragmentów wektora x między procesami
        ...
        // Zebranie wyników za pomocą MPI_Gather z użyciem MPI_IN_PLACE
        if (rank == 0) {
            MPI_Gather(MPI_IN_PLACE, n_wier, MPI_DOUBLE, z, n_wier, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        } else {
            MPI_Gather(z, n_wier, MPI_DOUBLE, NULL, n_wier, MPI_DOUBLE, 0, MPI_COMM_WORLD);
        }
